...
def spec_noise(gamma, omega, flag=True):
...
def spec_disturbance(gamma, omega, flag=True):
...
def spec_input_disturbance(gamma_d, omega_d, system, dB_flag=True):
    fig = plt.gcf()
...
def spec_tracking(gamma, omega, flag=True):
...
def spec_tracking_step(gamma, flag=True):
    # track step by gamma
    omega = 0.01
    w = np.logspace(np.log10(omega)-4, np.log10(omega))
    if flag is True:
...
def spec_tracking_ramp(gamma, flag=True):
    # track ramp by gamma
    omega = 0.01
    w = np.logspace(np.log10(omega)-4, np.log10(omega))
    if flag is True:
