...
class dataPlotter:
    def __init__(self):
        ...
        self.handle = []
        self.handle.append(myPlot(self.ax[0], ylabel='y', title='System Data'))
        self.handle.append(myPlot(self.ax[1], ylabel='ydot'))
        self.handle.append(myPlot(self.ax[2], xlabel='t(s)', ylabel='u'))

    def update(self, t, reference, states, ctrl):
        # update the time history of all plot variables
        self.time_history.append(t)  # time
        self.y_ref_history.append(reference)  
        self.y_history.append(states[0][0])  
        self.ydot_history.append(states[1][0])  
        self.u_history.append(ctrl)  # force on the base
        # update the plots with associated histories
        self.handle[0].update(self.time_history, [self.y_history, self.y_ref_history])
        self.handle[1].update(self.time_history, [self.ydot_history])
        self.handle[2].update(self.time_history, [self.u_history])

    def write_data_file(self):
        with open('io_data.npy', 'wb') as f:
            np.save(f, self.time_history)
            np.save(f, self.y_history)
            np.save(f, self.ydot_history)
            np.save(f, self.u_history)
