...
class dataPlotterObserver:
    def __init__(self):
        ...
        self.x1_history = []  # angle theta
        self.x1_hat_history = [] # estimate of theta
        self.x2_history = []
        self.x2_hat_history = []
        self.x3_history = []
        self.x3_hat_history = []        
        ...
        self.handle = []
        self.handle.append(myPlot(self.ax[0], ylabel='x1', title='Data'))
        self.handle.append(myPlot(self.ax[1], ylabel='x2', title='Data'))
        self.handle.append(myPlot(self.ax[2], ylabel='x3', title='Data'))
        self.handle.append(myPlot(self.ax[3], xlabel='t(s)', ylabel='d'))

    def update(self, t, x, x_hat, d, d_hat):
        # update the time history of all plot variables
        self.time_history.append(t)  # time
        self.x1_history.append(x[0,0])
        self.x2_history.append(x[1,0])
        self.x3_history.append(x[2,0])        
        self.x1_hat_history.append(x_hat[0,0])
        self.x2_hat_history.append(x_hat[1,0])
        self.x3_hat_history.append(x_hat[2,0])        
        self.d_history.append(d)
        self.d_hat_history.append(d_hat)

        # update the plots with associated histories
        self.handle[0].update(self.time_history, [self.x1_history, self.x1_hat_history])
        self.handle[1].update(self.time_history, [self.x2_history, self.x2_hat_history])
        self.handle[2].update(self.time_history, [self.x3_history, self.x3_hat_history])        
        self.handle[3].update(self.time_history, [self.d_history, self.d_hat_history])
